...
// Список событий с их типами аргументов
export interface AppEvents {
  'page:change': string;
  'user:login': Record<string, unknown>;
  'user:register': Record<string, unknown>;
  'user:logout': void;
  'user:updateProfile': Partial<User>;
  'user:changePassword': ChangePasswordData;
...
export default class AppController {
  ...
  public getEventBus(): EventBus<AppEvents> {
    return this.eventBus;
  }
  ...
  private async handleLogin(data: Record<string, unknown>): Promise<void> {
    try {
      this.setState({ isLoading: true });
      const user = await this.userService.login(data as any); // можно кастить к LoginData
      this.setState({ user, isLoading: false });
      this.eventBus.emit('page:change', 'chatPage');
    } catch (error) {
      this.setState({ isLoading: false });
  ...
  private async handleRegister(data: Record<string, unknown>): Promise<void> {
    try {
      this.setState({ isLoading: true });
      const user = await this.userService.register(data as any); // каст к RegistrationData
      this.setState({ user, isLoading: false });
      this.eventBus.emit('page:change', 'chatPage');
    } catch (error) {
      this.setState({ isLoading: false });
